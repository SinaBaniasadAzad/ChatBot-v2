...
> ⚠️ **این ارقام عکسِ‌فوریِ همان دیتاست‌اند، نه حقیقتِ ابدی.** اگر دیتاست بزرگ/عوض شد یا
> توزیعِ تیکت‌ها تغییر کرد، قبل از اتکا به هر درصدِ این بخش، آن را روی دادهٔ فعلی دوباره بسنج.
...
- ⚠️ **اصلاحِ خطرناک که برگردانده شد = دست‌کاریِ layer1.** SR-leaning‌کردنِ نوع، روی همان نمونه
  خطاهای لایه‌۱ را از **۲۵ به ۴۴** برد (۱۹ اصلاح، ۳۸ رگرسیون) و دقتِ کل را ۸۹→۸۴ کرد.
  **پیش‌فرضِ Incident-leaningِ layer1 را نگه دار؛** تغییر ممنوع نیست، ولی فقط از مسیرِ
  «روشِ امنِ بهبودِ layer1» در بخش ۲ (استثنای باریکِ پراِعتبار + مثالِ متضاد + ارزیابیِ
  رگرسیون روی همان seed) — نه leanِ سراسری.
...
- **روی خطاهای یک نمونهٔ کوچک overfit نکن؛** هر قاعده را در برابرِ **کلِ دیتاست**
  اعتبارسنجی کن، نه فقط فایلِ errors یک اجرا.
